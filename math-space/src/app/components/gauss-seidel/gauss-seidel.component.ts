...
export class GaussSeidelComponent implements OnInit {
  ...
  resolve() {
    ...
    if (!this.sending && this.matriz.length != 0) {
      ...
      this._appService.resolveGaussSeidel(body).subscribe(response => {
        this.sending = false;
        ...
        if (valid) {
          ...
          this.solution['solution'] = result.solution.split("\n").join(",").match(/\d+(?:\.\d+)?/g)?.map(Number);
          this.solution['matrizAx'] = result.matrizAx.split("\n").join(",").match(/\d+(?:\.\d+)?/g)?.map(Number);
          this.solution.converge = result.converge;
